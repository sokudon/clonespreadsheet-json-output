...
var data=["百位","25百位","5千位","1万位"];
var mkdt=["いべ時点","最終21時","7日17時","7日0時","6日17時","5日17時","4日17時","3日17時","2日17時","1日17時","0日17時",,"-1日17時"];
...
　var last_col = 2;
  ...
  ibeday[i][1]=(i)+moment(ibe[0][1]).add("hours",-17+2+24*i).format("日H時");
  ibeday[i+1][1]=(i)+moment(ibe[0][1]).add("hours",+2+24*i).format("日H時");
  ibeday[i+2][1]="最終"+moment(ibe[0][1]).add("hours",4+2+24*i).format("H時");
    }
    else{
  ibeday[i][0]=moment(ibe[0][1]).add("hours",2+24*i);
  ibeday[i][1]=(i)+moment(ibe[0][1]).add("hours",2+24*i).format("日H時");
    }}
...
var fm=["百位","25百位","5千位","1万位","しーと版いべたいま"];
var to=["百位","25百位","5千位","1万位","しあたいべたいま"];
  var tosp="1gm1hvpvTb5Ss9fZ53ocIcLDblNtumcC7k3KcSqwHllI";
var ss = SpreadsheetApp.openById(SCRIPT_PROP.getProperty("key")); 
var ss2 = SpreadsheetApp.openById(tosp); 
  for(var i=0;i<5;i++){
  var sheets = ss.getSheetByName(fm[i]);
  var sheets2 = ss2.getSheetByName(to[i]);
  var values= sheets.getRange(1,1,26 ,26).getValues();
   sheets2.getRange(1,1,26 ,26).setValues(values);
  }  
}
